...
int main() {
  ...
  if (shm_id == -1) {
    printf(
        "%d at %s: failed to create shared memory! error: %s\n", 
        __LINE__, 
        __FILE__,
        strerror(errno));
    exit(1);
  ...
  if ((int) Vendor == -1) {
    printf(
        "%d at %s: failed to attach shared memory to the local memory space! "
        "error: %s\n", 
        __LINE__, 
        __FILE__,
        strerror(errno));
...
void Producer() {
  ...
  for (next = 0; next < M; next++) {
    ...
    Vendor->next_empty = next_empty == SLOTS? 0 : next_empty;
    sem_post(Mutex);
    sem_post(Full);
  }
}
